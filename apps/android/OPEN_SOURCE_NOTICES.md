...
rclone dependency graphs produced with CGO enabled, the pinned repository entry point,
the private build module cache, the pinned Go distribution, the reviewed guardian source,
and Covalent's license. The generator records exact source hashes
...
The combined notice also attributes the bundled Android Graphics Path library and
records its exact AAR digest and Apache-2.0 license metadata.
