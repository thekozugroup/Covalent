...
final class HostedVoiceOverCapabilityProbe: XCTestCase {
    ...
    private func waitForVoiceOverEnablement(before deadline: Date) throws -> VoiceOverEnablement {
        let voiceOver = XCUIApplication(bundleIdentifier: "com.apple.VoiceOver")
        let welcome = voiceOver.staticTexts["Welcome to VoiceOver"]
        var welcomeObserved = false
        var welcomeAccepted = false
        while Date() < deadline {
            if NSWorkspace.shared.isVoiceOverEnabled {
                return VoiceOverEnablement(
                    enabled: true,
                    welcomeObserved: welcomeObserved,
                    welcomeAccepted: welcomeAccepted
                )
            }
            if welcome.waitForExistence(timeout: remaining(upTo: 0.5, before: deadline)) {
        ...
        return VoiceOverEnablement(
            enabled: NSWorkspace.shared.isVoiceOverEnabled,
            welcomeObserved: welcomeObserved,
            welcomeAccepted: welcomeAccepted
        )
    }
    ...
    private struct VoiceOverEnablement {
        let enabled: Bool
        let welcomeObserved: Bool
        let welcomeAccepted: Bool
    }
