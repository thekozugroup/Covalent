...
@Suite struct UserFacingCopyTests {
    ...
    ///
    /// Excluded, because they are code rather than copy: comment lines,
    /// accessibility identifiers, SF Symbol names, environment variable names,
    /// URLs, and bare dotted identifiers.
    private static func userFacingLiterals(in contents: String) -> [(line: Int, text: String)] {
        var results: [(Int, String)] = []
        for (index, line) in contents.components(separatedBy: .newlines).enumerated() {
            let trimmed = line.trimmingCharacters(in: .whitespaces)
            guard !trimmed.hasPrefix("//") else { continue }
            guard !line.contains("accessibilityIdentifier"), !line.contains("systemName:") else { continue }
            for text in stringLiterals(in: line) {
    ...
    /// Pulls the double-quoted literals out of one line, honouring backslash
    /// escapes so an escaped quote does not end a literal early.
    ...
    private static func viewSources() throws -> [URL] {
        ...
        var found: [URL] = []
        for platform in ["CovalentMac", "CovalentIOS"] {
