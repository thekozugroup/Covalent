...
| Beginner workflow | Install → connect → choose folder → protect → verify → restore through real UI; clear errors and recovery; no manual IDs in ordinary flows | Real browser unlock/list/Verify passed on disposable node; fixed fresh unpaired-node rejection. Full journey and removal of manual IDs pending. |
...
| Atmos network drill | Mac ↔ Ubuntu pairing, explicitly selected replica, interrupted/restarted operation, source-loss restore, exact hashes, cleanup | SSH read-only inventory passed; isolated drill being prepared |
| Atlas/Unraid | Trusted preflight, exact-image install/upgrade, selected-share permissions, real backup/restore | Preflight strengthened and fixtures pass; actual host evidence pending |
...
- Revised web suite: 70 passed, including two first-run provider-roster
  regressions. Runtime/OpenAPI method-path coverage still passes.
