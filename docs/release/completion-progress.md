...
Updated: 2026-09-12. Checkpoint 41 builds on published commit
`53ad71a64fca55b27703faf8bc20816d1f6808a1`. Checkpoint 40 hosted results
are complete; checkpoint 41 Android execution remains pending.
...
| 14 | The complete Android folder journey and foreground lifecycle work | Open | Checkpoint 40 passes Android foundation and all 75 baseline API 37 tests. Its native setup test fails 1/1 at incoming network pairing; permission-denied and restored phases do not execute. The runtime-directory correction gets past the previous startup failure. The test still supplied loopback while production signs its detected guest address; production binding validation correctly rejects that mismatch. Checkpoint 41 corrects the test endpoint and adds real native address-change coverage. Its full setup, transfer, repair, restart and removal execution must pass before acceptance. |
| 15 | Both complete Docker architectures install and run with safe writable sync mounts | Verified | Both complete Docker architectures pass package contracts, evidence extraction and all nine hardened-runtime checks in checkpoints 36 through 40: signed pairing, dual consent, full scan, transfer, pause/resume, retained identity on cold restart, offline withdrawal, signed remote delivery/acknowledgement, recipient restart, preserved files and exact-resource cleanup. Atlas uses this accepted Docker path. Vulnerability scans remain a separate open gate. |
...
| 18 | Shipped dependencies, notices and security gates are complete | Open | Checkpoint 40 passes dependency review with patched gRPC 1.83.2, Rust audit/license checks, both CodeQL languages, the zero-open-alert policy and release versions. Both Docker scans report zero High and three Medium CVE-2025-60876 findings in BusyBox 1.37.0-r30 packages. A separate unpublished arm64 backport passes package ownership and 13 wire tests; it has not been validated in both final images. Caddy/Alpine distribution verification and complete target classification remain open. Checkpoint 41 adds a six-object Android native distribution summary bound to both APKs and exact NDK notices; actual hosted aggregation is pending. No finding is waived. |
| 19 | Platform installation, upgrade, accessibility and end-to-end regression gates pass | Open | Checkpoint 40 passes Rust/contracts, Mac bundle/integration/UI, Android foundation, iOS, both Docker runtime/image-scan jobs, dependency review, both CodeQL languages, zero-open-alert policy and release versions. Its Android device job fails native pairing after 75 baseline tests pass, so the aggregate release gate fails. Full keyboard/spoken accessibility, personal-build upgrade, installation and the corrected Android regression remain unverified. |
