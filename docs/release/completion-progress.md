...
Updated: 2026-09-08. Checkpoint 35 builds on published commit
`920dadc65c66f0d0645bb629d7c146fafc425097`; evidence includes the integrated
...
| 14 | The complete Android folder journey and foreground lifecycle work | Open | Direct first-launch folder setup, phone-local SAS pairing, durable grants and serialized foreground lifecycle are integrated. Twelve JNI tests pass. Checkpoints 29 and 30 pass Android foundation and all 74 existing API 37 tests. Checkpoint 32 passes all 145 JVM tests and Android foundation. The device suite completed 59 of 76 tests, then Android killed the journey instrumentation process when its fixture changed MANAGE_EXTERNAL_STORAGE. The integrated host now requires all 75 other tests plus three independently validated phases of the same journey across actual process death/restart. Checkpoint 34 passes Android foundation and all 150 JVM tests. Its device preflight boots the emulator and verifies the current APKs, then stops because the script cannot discover the installed native-library directory; no device test executes in that run. The preflight now resolves the exact installed base APK through PackageManager and checks both installed helper hashes against the exact APK; four parser/manifest tests pass locally. Complete native journey, repair and the full device suite still require successful hosted execution. |
...
| 17 | Peer connectivity, invitation expiry/renewal, removal and address changes have complete user journeys | Open | Real paired workers now report connected, disconnected and reconnected while the surviving worker stays running. A second same-peer invitation and two-folder repair now pass a real 43-check proof after removing worker I/O from status polling. Renewal now passes a real 12-check two-service worker proof, including lost-response retry after source cold reopen, fresh recipient consent and unrelated-folder continuity. Native renewal handoff is implemented with bounded authenticated superseded IDs. Removal clients now distinguish durable local withdrawal from pending remote acknowledgement; Mac grants and pending repair bookmarks retire only on explicit authenticated removal IDs, including superseded IDs. Native renewal now passes 22 signed-app checks against a pinned TLS response fixture; it does not run the production backend. Remote removal passes 286 node tests and an eight-check real two-worker service proof, including lost acknowledgement, both cold reopens and unrelated-folder continuity. Both Docker gates now require actual signed removal delivery and durable acknowledgement after an offline peer reconnects. Native removal also passes a 16-check signed-app TLS-mock fixture, including persistent grant/repair retirement and failed-restart retry; actual helper retirement, the new Docker checks and network-address changes remain. |
| 18 | Shipped dependencies, notices and security gates are complete | Open | Checkpoint 28 Rust dependency checks and all CodeQL languages pass. Both complete Linux packages now include the deterministic source archives and pass checkpoint-33/34 Docker execution; both Android target inventories generate successfully. The Android notices asset reader passes actual device execution. The Mac target graph and all 105 notice files are verified. Its rebuilt worker passes real two-runtime execution. The exact Mac target now bundles five deterministic source archives (Syncthing and four MPL dependencies), totaling 8,408,088 compressed bytes, with visible exact source URLs and integrity metadata. Two independent builds reproduce all bytes. The Android NDK audit found missing toolchain notices and missing final-link input evidence. Its corrective build changes now retain exact final linker traces, maps, binary hashes and both pinned NDK notices. Ten provenance tests, 13 notice tests and a real Go external-link wrapper proof pass locally; both actual NDK/ABI builds still require hosted execution. Final multi-target classification and container findings remain. |
| 19 | Platform installation, upgrade, accessibility and end-to-end regression gates pass | Open | Fresh hosted gates and complete native/package acceptance are required. Checkpoint 34 passes Rust/contracts, both complete Docker jobs, Mac bundle/integration/UI, iOS Tier 2, Android foundation, dependency review, all CodeQL lanes and release versions. The corrected Android fixture compiles, but its device preflight stops before instrumentation when native-library path discovery returns no accepted directory; the complete device journey still requires execution. Native renewal has separate signed-app fixture evidence. Native removal/scope retirement, installation/upgrade and final accessibility still need the stated platform evidence. |
