...
Updated: 2026-09-08. Checkpoint 32 builds on published commit
`04a7137cf8b162f0a063280c06e74999a6d04789`; evidence includes the integrated
...
| 18 | Shipped dependencies, notices and security gates are complete | Open | Checkpoint 28 Rust dependency checks and all CodeQL languages pass. Linux notices build on both architectures; both Android target inventories generate successfully. The Android notices asset reader passes actual device execution. The Mac target graph and all 105 notice files are verified. Its rebuilt worker passes real two-runtime execution. Four MPL dependencies still need explicit recipient source access; final multi-target classification and container findings remain. |
| 19 | Platform installation, upgrade, accessibility and end-to-end regression gates pass | Open | Fresh hosted gates and complete native/package acceptance are required. Checkpoint 31's unchanged iOS Home audit passes with the Refresh tint correction. Linux and Android test-fixture corrections need fresh hosted validation. |
