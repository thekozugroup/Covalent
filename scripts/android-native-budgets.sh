...
#
# The one real, lossless reduction found was switching [profile.release] from
# `lto = "thin"` to `lto = "fat"` in the workspace Cargo.toml, which took
# arm64-v8a from 9,260,232 to 8,369,248 (-9.6%) and x86_64 from 10,952,432 to
# 9,942,680 (-9.2%). The numbers below are measured *after* that reduction.
#
#   toolchain: rustc 1.97.1, Android NDK 27.1.12297006, cargo-ndk 4.1.2
#   arm64-v8a  8,369,248 bytes
#   x86_64     9,942,680 bytes   <- worst ABI, the budget is derived from this
#
# Ceiling = worst measured ABI + 12%, rounded up to the next 64 KiB:
...
# 1.2 MB of headroom on the worst ABI, which is under one release's worth of
...
# So there is also a floor, at 4 MiB: 5.6x above the broken artefact and 2.0x
...
# APK is universal and carries both ABIs: 8,369,248 + 9,942,680 = 18,311,928
...
# user actually downloads is therefore ~8.4 MB of native code on any arm64
...
#
# Of that APK, 18,311,928 bytes (91.4%) is the two native libraries, which the
# per-ABI ceiling above already governs. Only the remaining 1,728,600 bytes are
# dex, resources, manifest and signing blocks. So the package budget is derived
# from the per-ABI ceiling rather than being an independent guess:
#
