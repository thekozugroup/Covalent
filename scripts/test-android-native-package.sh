...
payloads = {
    ...
    "libcovalentrclone.so": b"s" * (16 * 1024 * 1024),
...
def make(name, bad_hash=False, omit_guardian=False, duplicate_worker=False, bad_notice=False):
    ...
    for library, payload in payloads.items():
        ...
        for abi in abis:
            ...
            if bad_hash and library == "libcovalentrclone.so" and abi == "arm64-v8a":
                digest = "0" * 64
    ...
    with zipfile.ZipFile(root / name, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("assets/rclone-sha256.txt", manifests["libcovalentrclone.so"])
        ...
        if duplicate_worker:
            with warnings.catch_warnings():
                ...
                archive.writestr("lib/arm64-v8a/libcovalentrclone.so", payloads["libcovalentrclone.so"])
